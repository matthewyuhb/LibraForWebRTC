...
class NS3Env:
    def __init__(self, zmq_path = None) -> None:
        # 初始化用到的常量参数
        ...
        if zmq_path is not None:
            self.zmq_path = f'ipc:///tmp/{zmq_path}'
        else:
            self.zmq_path = _ZMQ_PATH
        self.socket.bind(self.zmq_path)
        self.gym = None
    ...
    def reset(self, trace_path: str, cc_type: CCType, duration: int) -> None:
        ...
        self.process_args = [
            ...
            f"--zmq_path={self.zmq_path}"
        ]
        logging.info(f"Starting new gym process with args: {self.process_args}")
        # 启动NS3环境
