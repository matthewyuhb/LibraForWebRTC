...
k_max_bitrate = 25000000  #12800000 #1.6MB/s 12800 kbps 需要修改
...
class delay_base_bwe:
    def __init__(self):
        ...
        self.first_ts = 0#get_time_ms()  # 这个需要暴露一个接口后续外部更改
    ...
    def __delay_bwe_update(self, now_ts, acked_bitrate, overusing):
        ...
        # print("delaybasedbwe.py:acked_bitrate:"+str(acked_bitrate))
        input_noise_var = 0
    ...
    def __delay_bwe_maybe_update(self, overusing, acked_bitrate,
        ...
        if overusing == 0:
            ...
            # print("underuse! overusing:"+str(overusing))
            result.updated, result.bitrate = self.__delay_bwe_update(
                now_ts, acked_bitrate, overusing)
    ...
    def delay_bwe_incoming(self, packet_list, acked_bitrate, now_ts):
        ...
        if delay_feedback == 0:  #太多次网络feedback事件出现重复，强制的带宽减半并返回
            # print("太多次网络feedback事件出现重复，强制的带宽减半并返回")
    ...
    def get_trendline_slope(self):
        return self.trendline_estimator.trendline_slope()