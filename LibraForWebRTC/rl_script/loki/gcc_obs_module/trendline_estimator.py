...
class TrendlineEstimator:
    ...
    def __linear_fit_slope(self):  #
        ...
        # print("matthew:self.arrival_delta_list:"+str(self.arrival_delta_list))
        # print("matthew:self.smoothed_delay_list:"+str(self.smoothed_delay_list))
        numerator = np.sum(
            list(
    ...
    def trendline_update(self, recv_delta_ms, send_delta_ms, arrival_ts):
        ...
        # print("matthew:delta_ms:"+str(delta_ms))
        ...
        # print("matthew:acc_delay:"+str(self.acc_delay))
        # 窗口更新；实际上只记录窗口里面的值
        self.index += 1
        ...
        # print("matthew:trendline:",self.trendline)
    ...
    def reset(self, window_size=20, smoothing_coef=0.9, threshold_gain=4.0):
        # 重新初始化
        # 下面三个需要初始化处理
        # 对累计延迟梯度平滑值进行最小二乘法线性回归之后求得延迟梯度趋势，会乘以增益并和阈值作比较，以检测带宽使用状态
        self.window_size = window_size  # 窗口大小;窗口大小决定收到多少包组之后开始计算延迟梯度趋势
