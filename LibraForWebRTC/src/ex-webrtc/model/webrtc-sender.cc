...
bool WebrtcSender::SendRtp(const uint8_t* packet,
    ...
    //     if (!m_traceBw.IsNull()) {
    //         m_traceBw(now, m_call->last_bandwidth_bps());
    //     }
    //     // if (!m_traceRtt.IsNull()) {
    //     //     m_traceRtt(now, stats.rtt_ms);
    //     // }
    // }
    // std::cout<<"matthew:output:"<<output<<" m_traceBw.IsNull():"<<m_traceBw.IsNull()<<std::endl;
    if(output&&!m_traceBw.IsNull()){
        // uint32_t bw=m_call->last_bandwidth_bps();
        webrtc::Call::Stats stats = m_call->GetStats();
        
        // std::cout<<"matthew:sender.cc,tracing.."<<std::endl;
        uint32_t bw = stats.send_bandwidth_bps;
        m_traceBw(now,bw);
        if (!m_traceRtt.IsNull()) {
            m_traceRtt(now, stats.rtt_ms);
        }
...
void WebrtcSender::RecvPacket(Ptr<Socket> socket){
    ...
    // printf("matthew:WebRTC Time:%s\n",ToString(m_clock->CurrentTime()).c_str());
    NS_LOG_INFO("node:"<<m_socket->GetNode()<<" matthew:Current Time:"<< ToString(m_clock->CurrentTime()));
