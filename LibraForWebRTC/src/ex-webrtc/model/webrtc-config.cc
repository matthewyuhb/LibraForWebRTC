...
WebrtcSessionManager::WebrtcSessionManager(
    ...
    std::shared_ptr<webrtc::NetworkStateEstimatorFactory> se_factory):time_controller_(controller)
{
...
void WebrtcSessionManager::SetCcFactory(std::shared_ptr<webrtc::NetworkControllerFactoryInterface> cc_factory){
    call_client_config_.transport.cc_factory = cc_factory.get();
}

webrtc::TimeController *CreateTimeController(TimeControllerType type,int64_t start_us,uint64_t stop_us){
    if(TimeControllerType::SIMU_CONTROLLER==type){
