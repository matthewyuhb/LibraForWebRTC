...
bool IsEnabled(const WebRtcKeyValueConfig* config, absl::string_view key) {
  return false;//absl::StartsWith(config->Lookup(key), "Enabled");
}

bool IsNotDisabled(const WebRtcKeyValueConfig* config, absl::string_view key) {
  return false;//!absl::StartsWith(config->Lookup(key), "Disabled");
}
}  // namespace

OrcaCcNetworkControllerProxy::OrcaCcNetworkControllerProxy(webrtc::NetworkControllerConfig config,webrtc::GoogCcConfig goog_cc_config,GymConnector &conn) :
    gym_conn_(conn),key_value_config_(config.key_value_config ? config.key_value_config
                                                : &trial_based_config_),
...
webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnNetworkAvailability(webrtc::NetworkAvailability msg) {
    // if(gym_conn_.GetWState()==WLibraState::EVALUATION){
    //   return GetUpdate(msg.at_time);
    // }
    NetworkControlUpdate update;
    update.probe_cluster_configs = probe_controller_->OnNetworkAvailability(msg);
    // gym_conn_.SetNcu(update);
    return update;
    // 
}

webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnNetworkRouteChange(webrtc::NetworkRouteChange msg) {
    // if(gym_conn_.GetWState()==WLibraState::EVALUATION){
    //   return GetUpdate(msg.at_time);
    // }
...
void OrcaCcNetworkControllerProxy::ClampConstraints() {
...
std::vector<ProbeClusterConfig> OrcaCcNetworkControllerProxy::ResetConstraints(
...
void OrcaCcNetworkControllerProxy::MaybeTriggerOnNetworkChanged(
    NetworkControlUpdate* update,
    Timestamp at_time) {
...
webrtc::PacerConfig OrcaCcNetworkControllerProxy::GetPacingRates(Timestamp at_time) const {
  // Pacing rate is based on target rate before congestion window pushback,
  // because we don't want to build queues in the pacer when pushback occurs.
  // std::cout<<"GetPacingRates!"<<std::endl;
...
void OrcaCcNetworkControllerProxy::UpdateCongestionWindowSize() {
  // std::cout<<"matthew:UpdateCongestionWindowSize!!"<<std::endl;
  ...
  // std::cout<<"matthew:set gym data window:"<<data_window.bytes()<<std::endl;
  // gym_conn_.SetDataWindow(data_window);
}

webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnProcessInterval(webrtc::ProcessInterval msg) {
    // std::cout<<"matthew:OnProcessInterval!"<<std::endl;
    // if(gym_conn_.GetWState()==WLibraState::EVALUATION){
    //   return GetUpdate(msg.at_time);
    // }
    NetworkControlUpdate update;
  //初始化config
  if (initial_config_) {
    // std::cout<<"initial_config_"<<std::endl;
  ...
  if (congestion_window_pushback_controller_ && current_data_window_) {
    ...
    // printf("matthew:[1]congestion_window set %ld\n",current_data_window_->bytes());
...
webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnRemoteBitrateReport(webrtc::RemoteBitrateReport msg) {
    // if(gym_conn_.GetWState()==WLibraState::EVALUATION){
...
webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnRoundTripTimeUpdate(webrtc::RoundTripTimeUpdate msg) {
    // if(gym_conn_.GetWState()==WLibraState::EVALUATION){
    //   return GetUpdate(msg.receive_time);
    // }
...
webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnSentPacket(webrtc::SentPacket sent_packet) {
    // if(gym_conn_.GetWState()==WLibraState::EVALUATION){
...
webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnReceivedPacket(webrtc::ReceivedPacket received_packet) {
    // if(gym_conn_.GetWState()==WLibraState::EVALUATION){
...
webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnStreamsConfig(webrtc::StreamsConfig msg) {
    // if(gym_conn_.GetWState()==WLibraState::EVALUATION){
    //   return GetUpdate(msg.at_time);
    // }
    NetworkControlUpdate update;
...
webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnTargetRateConstraints(webrtc::TargetRateConstraints constraints) {
    // if(gym_conn_.GetWState()==WLibraState::EVALUATION){
...
webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnTransportLossReport(webrtc::TransportLossReport msg) {
    // if(gym_conn_.GetWState()==WLibraState::EVALUATION){
    //   return GetUpdate(msg.receive_time);
    // }
    if (packet_feedback_only_)
...
webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnTransportPacketsFeedback(webrtc::TransportPacketsFeedback report) {
  ...
  //add for orca
  if(gym_conn_.GetRlUpdate()){
      // gym_conn_.SetRlUpdate(false);
      webrtc::NetworkControlUpdate tempUpdate = GetUpdate(report.feedback_time);
      probe_bitrate = tempUpdate.target_rate->target_rate;
      // return ;
  }
  ...
  if (congestion_window_pushback_controller_ && current_data_window_) {
    ...
    // printf("matthew:[2]congestion_window set %ld\n",current_data_window_->bytes());
    update.congestion_window = current_data_window_;
  }

  // if(update.pacer_config){
  //   std::cout<<"matthew:data_window:"<<update.pacer_config->data_window.bytes()<<std::endl;
  // }
  if(update.target_rate){
    // std::cout<<"matthew:setting target_rate:"<<update.target_rate->target_rate.bytes_per_sec()<<std::endl;
    gym_conn_.SetDataRate(update.target_rate->target_rate);
  }
  if(gym_conn_.GetRlUpdate()){
      gym_conn_.SetRlUpdate(false);
      // webrtc::NetworkControlUpdate tempUpdate = ;
      // probe_bitrate = tempUpdate.target_rate->target;
...
webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::OnNetworkStateEstimate(webrtc::NetworkStateEstimate msg) {
    // if(gym_conn_.GetWState()==WLibraState::EVALUATION){
...
NetworkControlUpdate OrcaCcNetworkControllerProxy::GetNetworkState(
...
webrtc::NetworkControlUpdate OrcaCcNetworkControllerProxy::GetUpdate(webrtc::Timestamp at_time) const {
